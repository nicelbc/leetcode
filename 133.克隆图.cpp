    ...
    Node* cloneGraph(Node* node) {
        ...
        while(!que.empty()){
            ...
            for (int i = 0; i < tmp->neighbors.size(); i++){
                //如果遍历临接表，如果未访问，入队列  并且克隆加入哈希表中
                if(!visited.count(tmp->neighbors[i])){
                    que.push(tmp->neighbors[i]);
                    visited[tmp->neighbors[i]] = new Node(tmp->neighbors[i]->val);
                }
                //更新当前节点的临接表
                visited[tmp]->neighbors.push_back(visited[tmp->neighbors[i]]);
            }
        }
        return visited[node];
    }
